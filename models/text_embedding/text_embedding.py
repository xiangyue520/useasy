...
class UseasyTextEmbeddingModel(_CommonUseasy, TextEmbeddingModel):
    """
    ...
    def embed_documents(
        ...
        """Call out to Useasy's embedding endpoint.
        ...
        if UseasyTextEmbeddingModel._is_vision_model(model):
            documents = [MultiModalContent(content_type=MultiModalContentType.TEXT, content=text) for text in texts]
            return UseasyTextEmbeddingModel.embed_multimodal_documents(
                credentials_kwargs,
                model,
                documents,
                base_address,
